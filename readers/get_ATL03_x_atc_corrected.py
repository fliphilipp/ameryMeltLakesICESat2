...
def get_ATL03_x_atc_corrected(IS2_atl03_mds, IS2_atl03_attrs, IS2_atl03_beams):
    ...
    for gtx in sorted(IS2_atl03_beams):
        ...
        val['heights']['x_atc'] = np.float64(np.zeros_like(val['heights']['h_ph'])+np.NaN)
        ...
        for j in segment_indices:
            ...
            distance_along_X = np.float64(np.copy(val['heights']['dist_ph_along'][idx:idx+cnt]))
